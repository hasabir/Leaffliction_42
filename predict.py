...
def predict(model_path, image_path, class_names_path):
    ...
    try:
        ...
        fig.suptitle(f'The predicted class is => {class_names[predicted_class_idx]}',
...
def main():
    try:
        if len(sys.argv) < 2:
            print(
                "Usage: python predict.py <image_path> [model_packege_dir_or_zip]\n"
                "  <image_path>                 Path to the image to classify (required)\n"
                "  [model_packege_dir_or_zip]   Optional: directory containing 'plant_disease_model.h5' and 'class_names.json',\n"
                "\n"
                "Examples:\n"
        ...
        # model_path = 'plant_disease_model_3.h5'
