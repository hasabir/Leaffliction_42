...
class Augmentation:
    ...
    def save_augmented_images(
            self, augmentation, base_file_path, output_dir="."):
        ...
        if self.rotated is not None and augmentation == 'rotate':
            ...
            print("Warning: No ", augmentation, end=" ")
            print("augmentation found for", base_file_path)
        return
    ...
    def show_images(self, target_height=300):
        ...
        # Create montage
        montage = cv2.hconcat([orig, rotated, fliped,
                               skewed, blured, contrasted, illuminated])

        # Display
        cv2.imshow("Original | Rotated 45 | Flipped | Skewed \
            | Blured | Contrasted | Illuminated", montage)
        cv2.waitKey(0)
