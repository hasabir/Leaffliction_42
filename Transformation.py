...
class Transformation:
    ...
    def color_histogram(self, dst_path=None):
        ...
        if dst_path:
            ...
            for color, histogram in hist_data:
                color_name = 'Blue' if color == 'b' else 'Green' if color == 'g' else 'Red'
    ...
    def display_all_transformations(self):
        ...
        fig.suptitle(f'Image Transformations: {self.filename}{self.extension}', fontsize=16)
    ...
    def save_transformed_images(self, transformation, base_file_path, output_dir=".", src_base_dir=None):
        ...
        if transformation == 'histogram_data' and self.histogram_data is not None:
            ...
            if img_data is not None:
                ...
                print(f"  ✗ Warning: No {transformation} transformation found for {base_file_path}")
...
def main():
    ...
    # Case 3: Directory with --src only - display error (need --dst)
        ...
        print("Usage: python script.py --src <source_directory> --dst <destination_directory>")
        sys.exit(1)
    
    # Case 4: Directory processing with --src and --dst - save all transformations
    ...
    # Case 5: Invalid arguments
        ...
        print("\nUsage:")
        print("  Display transformations: python script.py --file <image_path>")
        print("  Save single image:       python script.py --file <image_path> --dst <output_dir>")
        print("  Batch process:           python script.py --src <source_dir> --dst <output_dir>")
